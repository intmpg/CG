...
namespace
{
const glm::vec2 QUAD_TOPLEFT = { -370, 180 };
const glm::vec2 QUAD_SIZE = { 100, 100 };
...
{
    const glm::vec4 GREEN = { 0.93f, 0.1f, 0.14f, 1.f };
    GetWindow().SetBackgroundColor(GREEN);
    CheckOpenGLVersion();
    SetupOpenGLState();

    const std::string vertexShader = CFilesystemUtils::LoadFileAsString("res/copytexture.vert");
    const std::string checkersShader = CFilesystemUtils::LoadFileAsString("res/ussr.frag");
    ...
    m_programQueue = { &m_programCheckers };
}
